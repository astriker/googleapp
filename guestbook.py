...
def guestbook_key(guestbook_name=DEFAULT_GUESTBOOK_NAME):
    ...
    We use geustbook_name as the key.
    """
...
class MainPage(webapp2.RequestHandler):
    def get(self):
        ...
        sign_query_params = urllib.urlencode({'guestbook_name': guestbook_name, 'dummy': 'dum my'})
...
class Guestbook(webapp2.RequestHandler):
    def post(self):
        ...
        if users.get_current_user():
            greeting.author = Author(identity=users.get_current_user().user_id(),
                                     email=users.get_current_user().email())

        greeting.content = self.request.get('content')
        greeting.put()

        query_params = {'guestbook_name': guestbook_name, 'dummy': dummy}
